...
public class GestureDrawline extends View {
    ...
    public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
        ...
        super(context);
        //屏幕的宽度和高度
        int[] screenDispaly = DensityUtil.getScreenDispaly(context);
        paint = new Paint(Paint.DITHER_FLAG);// 创建一个画笔
        bitmap = Bitmap.createBitmap(screenDispaly[0] - DensityUtil.dp2px(context, 114), screenDispaly[0] - DensityUtil.dp2px(context, 114), Bitmap.Config.ARGB_8888); // 设置位图的宽高
        canvas = new Canvas();
        canvas.setBitmap(bitmap);
        paint.setStyle(Style.STROKE);// 设置非填充
        paint.setStrokeWidth(8);// 笔宽4像素
        paint.setColor(Color.rgb(188, 165, 90));// 设置默认连线颜色
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                clearScreenAndDrawList(lineList);
    ...
    public void clearDrawlineState(long delayTime) {
        if (delayTime > 500) {
            // 绘制红色提示路线
            isDrawEnable = false;
            if (passWordSb.length() == 1) {
                drawErrorPathTip(currentPoint);
            } else {
                drawErrorPathTip(lineList);
            }
        }
        new Handler().postDelayed(new clearStateRunnable(), delayTime);
    }

    public void setDrawLineVerifyState(boolean isVerify, String mPassword) {
        this.isVerify = isVerify;
    ...
    public void clearCanvas() {
        clearScreenAndDrawList(new ArrayList<Pair<GesturePoint, GesturePoint>>());
     ...
     *
     * @param x actionDown X
     * @param y actionDown Y
     ...
     */
    private void clearScreenAndDrawList(List<Pair<GesturePoint, GesturePoint>> pairList) {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        for (Pair<GesturePoint, GesturePoint> pair : pairList) {
     ...
     * 校验错误/两次绘制不一致提示
     */
    private void drawErrorPathTip(List<Pair<GesturePoint, GesturePoint>> pairList) {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        paint.setColor(Color.rgb(252, 53, 87));// 设置默认线路颜色
        for (Pair<GesturePoint, GesturePoint> pair : pairList) {
     ...
     * 单点错误展示
     */
    private void drawErrorPathTip(GesturePoint gesturePoint) {
        canvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
        gesturePoint.setPointState(Constants.POINT_STATE_WRONG);
        invalidate();
    }
